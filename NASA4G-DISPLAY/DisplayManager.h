...
public:
  ...
  void displayConfirmSaveExit(int choice);
  void forceRedrawSettingsScreen();

  int tempMin = 20;
  int tempMax = 40;
  char lastMinText[32];
char lastMaxText[32];
char lastDriveAlertText[32];   // thêm cho Drive Alert
bool driveAlertEnabled;        // biến ON/OF; 
  void clearLastText();
  void clearTextField(char* lastText, int x, int y, const GFXfont* font, const uint16_t* bgImage, int bgImageWidth);

  int totalScreens = 3;
  int currentScreenIndex = 0;
  void resetDriveAlertText();
